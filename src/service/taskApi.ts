...
export async function crateTaskRequest(taskData: TaskFormData): Promise<Task> {
...
export async function updateTaskRequest(id : number,
    taskData : TaskFormData){
        ...
        if(!response.ok){
            const message = await extractErrorMessage(
                response, "Erro ao atualizar tarefa");
            throw new Error(message);
        }
        return response.json();
    }
